...
- Public surface is narrow: `SecretsManagerExtensions` (3 `AddSecretsManager` overloads), `SecretsManagerConfigurationProviderOptions` (11 properties), `SecretsManagerConfigurationProvider`, `SecretsManagerConfigurationSource` / `SecretsManagerConfigurationSourceWithLogger`, `SecretValueContext`, `MissingSecretValueException`. Full member-level detail is in §5 (API reference source material) below.
...
4. **Options quick-reference** — condensed table of all 11 options (name, one-line purpose, when to set it) — full depth intentionally lives in the docs site, not duplicated here.
