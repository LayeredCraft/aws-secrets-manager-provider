...
- **`.gitignore` rules** (exact, added to root `.gitignore`, reusing the reference repo's convention verbatim):
  ```gitignore
  # Agent Skill eval workspace: raw per-run outputs and snapshots are regenerated by each
  # run and not committed. grading.json/timing.json/benchmark.json/feedback.json ARE
  # committed as evaluation evidence.
  skills/aws-secrets-manager-provider-workspace/iteration-*/**/outputs/
  skills/aws-secrets-manager-provider-workspace/skill-snapshot/
  ```
  This must NOT match `skills/aws-secrets-manager-provider/evals/evals.json` or `skills/aws-secrets-manager-provider/evals/files/` (authored, always committed) — verify with `git check-ignore` during validation (§9).
...
| `.gitignore` | Verified both directions via `git check-ignore -v`: `iteration-*/**/outputs/` and `skill-snapshot/` ARE ignored; `evals/evals.json` and `evals/files/.gitkeep` are NOT ignored; `.venv/` and `site/` (docs tooling) also added and verified ignored | ✅ done |
